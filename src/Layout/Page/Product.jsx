...
function Product() {
    ...
    useEffect(() => {
        const handler = async () => {
            try {
                const response = await fetchData.getData(process.env.REACT_APP_APi_URL + '/product/where/' + param.get('product'));
                setData(response);
                SetLoading(true);
            } catch (error) {
        ...
        <div className="p-4 md:p-8">
            ...
            <div className="md:flex">
                <div className="md:w-1/2">
                    {
                        loading && (
                            <img src={process.env.REACT_APP_API_IMG + '/uploads/' + data.image_data} alt="Nike Zoom Vapor Pro 2 HC ‘White’ DR6191-101" className="w-3/4 h-1/2 mb-4" />
                        )
                    }
                ...
                <div className="md:w-1/2 md:pl-8">
                    ...
                    <button className="w-full bg-pink-200 text-white py-3 rounded mb-4 hover:bg-pink-500" onClick={handler}>THÊM VÀO GIỎ HÀNG</button>
